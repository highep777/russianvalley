...
@section('title',  isset($post) ? "Редактировать пост ID {$post->id}" : 'Добавить пост')

@section('content')
    <div class="container mx-auto px-6 py-8">
        <h3 class="text-gray-700 text-3xl font-medium">{{ isset($post) ? "Редактировать пост ID {$post->id}" : 'Добавить пост' }}</h3>
        ...
        <div class="mt-8">
            <form enctype="multipart/form-data" class="space-y-5 mt-5" method="POST" action="{{ isset($post) ? route("admin.posts.update", $post->id) : route("admin.posts.store") }}">
                ...
                <textarea name="description" class="w-full h-12 border border-gray-800 rounded px-3 @error('description') border-red-500 @enderror" placeholder="Описание" value="{{ $post->description ?? '' }}" ></textarea>
